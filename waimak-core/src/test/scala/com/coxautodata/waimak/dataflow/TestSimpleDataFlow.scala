  ...
  describe("Commits configuration") {
    ...
    it("no commits") {
      intercept[java.util.NoSuchElementException] {
        flow.extensionMetadata(CommitExtension[MockDataFlow])
      }

      val emptyLabels = flow
        .commit("test")()

      emptyLabels.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.isEmpty should be(true)
      emptyLabels.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.keySet should be(Set("test"))
    ...
    describe("one commit, no push") {
      ...
      it("one label, no partitions") {
        ...
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.size should be(1)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.get("commit_1") should be(Some(Seq(CommitEntry("com_1", "commit_1", None, repartition = false, cache = true))))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.isEmpty should be(true)
      ...
      it("one label, partitions, no repartition") {
        ...
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.size should be(1)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.get("commit_1") should be(Some(Seq(CommitEntry("com_1", "commit_1", Some(Left(Seq("id", "cntry"))), repartition = false, cache = true))))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.isEmpty should be(true)
        testFlow.tagState.activeTags.isEmpty should be(true)
      }
      ...
      it("one label, partitions, repartition") {
        ...
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.size should be(1)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.get("commit_1") should be(Some(Seq(CommitEntry("com_1", "commit_1", Some(Left(Seq("id", "cntry"))), repartition = true, cache = true))))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.isEmpty should be(true)
        testFlow.tagState.activeTags.isEmpty should be(true)
      }

      it("one label, partitions, repartition, no cache") {
        import CommitExtension._
        flow.flowContext.asInstanceOf[EmptyFlowContext].conf.setProperty(CACHE_REUSED_COMMITTED_LABELS, "false")
        val testFlow = flow.commit("commit_1", Seq("id", "cntry"))("com_1")
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.size should be(1)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.get("commit_1") should be(Some(Seq(CommitEntry("com_1", "commit_1", Some(Left(Seq("id", "cntry"))), repartition = true, cache = false))))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.isEmpty should be(true)
        testFlow.tagState.activeTags.isEmpty should be(true)
      }
    }
    ...
    describe("pushes, no commits") {
      ...
      it("one push") {
        ...
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.isEmpty should be(true)
        testFlow.tagState.activeTags.isEmpty should be(true)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.keySet should be(Set("commit_1"))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes("commit_1").size should be(1)
      }
      ...
      it("one push, accept multiple committers") { //it should accept, but it will fail validation
        ...
        testFlow.tagState.activeTags.isEmpty should be(true)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.keySet should be(Set("commit_1"))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes("commit_1").size should be(2)
      }
      ...
      it("2 pushes") {
        ...
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].commits.isEmpty should be(true)
        testFlow.tagState.activeTags.isEmpty should be(true)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes.keySet should be(Set("commit_1", "commit_2"))
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes("commit_1").size should be(1)
        testFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType[CommitMeta[MockDataFlow]].pushes("commit_2").size should be(1)
      }
    ...
    describe("build committers, no validations") {
      ...
      it("single commit, single label") {
        ...
        val testFlow = CommitExtension[MockDataFlow].buildCommits(commitFlow, commitFlow.extensionMetadata(CommitExtension[MockDataFlow]).getMetadataAsType)
